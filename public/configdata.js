...
// Load top racers
window.loadTopRacers = async () => {
    try {
        if (topRacersLoading) return;

        topRacersLoading = true;
        const container = document.getElementById('top-racers-container');

        // Hiển thị loading
        if (container) {
            container.innerHTML = `
                        <div class="loading-racers loading-pulse">
                            <div class="loading-spinner mx-auto mb-4"></div>
                            <p class="text-slate-500">Đang phân tích dữ liệu tay đua...</p>
                        </div>
                    `;
        }

        // Lấy dữ liệu từ gameMaps
        const mapsRef = collection(db, "gameMaps");
        const mapsSnapshot = await getDocs(mapsRef);

        // Tạo object để đếm số kỷ lục của mỗi tay đua
        const racerStats = {};

        mapsSnapshot.forEach(doc => {
            const mapData = doc.data();
            const racerName = mapData.recordRacer;

            if (racerName && racerName.trim() !== '' && racerName !== 'N/A') {
                if (!racerStats[racerName]) {
                    racerStats[racerName] = {
                        name: racerName,
                        recordCount: 0,
                        maps: [],
                        bestTime: null,
                        car: mapData.recordCar || 'Chưa xác định',
                        pet: mapData.recordPet || 'Chưa xác định'
                    };
                }

                racerStats[racerName].recordCount++;
                racerStats[racerName].maps.push({
                    name: mapData.name || 'Không tên',
                    time: mapData.recordTime,
                    difficulty: mapData.difficulty
                });

                // Cập nhật thời gian tốt nhất
                if (mapData.recordTime && mapData.recordTime !== '--\'--\'--') {
                    const currentTime = parseRaceTime(mapData.recordTime);
                    if (!racerStats[racerName].bestTime ||
                        currentTime < parseRaceTime(racerStats[racerName].bestTime)) {
                        racerStats[racerName].bestTime = mapData.recordTime;
                    }
                }
            }
        });
        ...
        const racerArray = Object.values(racerStats);
        racerArray.sort((a, b) => b.recordCount - a.recordCount);

        // Chỉ lấy top 6 tay đua
        topRacers = racerArray.slice(0, 6);

        // Render top racers
        renderTopRacers();

        topRacersLoading = false;

    } catch (error) {
        console.error("Error loading top racers:", error);
        const container = document.getElementById('top-racers-container');
        if (container) {
            container.innerHTML = `
                        <div class="text-center py-8 text-red-500">
                            <i class="fas fa-exclamation-triangle text-3xl mb-4"></i>
                            <p>Lỗi khi tải dữ liệu tay đua</p>
                            <p class="text-sm mt-2">${error.message}</p>
                            <button onclick="loadTopRacers()" class="btn-secondary mt-4">
...
// Render top racers
window.renderTopRacers = () => {
    const container = document.getElementById('top-racers-container');
    if (!container) return;

    if (!topRacers || topRacers.length === 0) {
        container.innerHTML = `
                    <div class="text-center py-8 text-slate-500 col-span-full">
                        <i class="fas fa-user-slash text-4xl mb-4"></i>
                        <p>Chưa có dữ liệu tay đua</p>
                        <p class="text-sm mt-2">Chưa có tay đua nào nắm giữ kỷ lục</p>
                        <button onclick="loadTopRacers()" class="btn-secondary mt-4">
    ...
    topRacers.forEach((racer, index) => {
        ...
        const firstLetter = racer.name ? racer.name.charAt(0).toUpperCase() : '?';
        const avatarColor = getAvatarColor(racer.name || 'Unknown');
        const bestTimeDisplay = racer.bestTime && racer.bestTime !== 'null' ?
            racer.bestTime : '--\'--\'--';

        const racerCard = document.createElement('div');
        racerCard.className = 'top-racer-card';
        racerCard.innerHTML = `
                    <div class="racer-rank ${rankClass}">
                        ${rank === 1 ? '🥇' : rank === 2 ? '🥈' : rank === 3 ? '🥉' : rank}
                    </div>
                    
                    <div class="racer-info">
                        <div class="racer-avatar" style="background: ${avatarColor}">
                            ${firstLetter}
                        </div>
                        <div>
                            <div class="racer-name">${racer.name || 'Không rõ'}</div>
                            <div class="racer-email">
                                <i class="fas fa-car mr-1"></i> ${racer.car} 
                                <i class="fas fa-paw ml-3 mr-1"></i> ${racer.pet}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="racer-stats">
                        <div class="stat-item">
                            <div class="stat-value">${racer.recordCount}</div>
                            <div class="stat-label">Kỷ lục</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-value">${racer.maps ? racer.maps.length : 0}</div>
                            <div class="stat-label">Bản đồ</div>
                        </div>
                        <div class="stat-item">
                            <div class="stat-value">${bestTimeDisplay}</div>
                            <div class="stat-label">Tốt nhất</div>
                        </div>
                    </div>
                    
                    ${racer.maps && racer.maps.length > 0 ? `
                        <div class="mt-4 pt-4 border-t border-slate-800">
                            <div class="space-y-2">
                                ${racer.maps.slice(0, 3).map(map => `
                                    <div class="bg-slate-800/30 p-2 rounded-lg hover:bg-slate-800/50 transition-colors">
                                        <div class="flex justify-between items-center">
                                            <div>
                                                <div class="font-medium text-sm">${map.name || 'Không tên'}</div>
                                                <div class="text-xs text-slate-400 mt-1">
                                                    <span class="px-1.5 py-0.5 rounded text-xs ${getDifficultyColorClass(map.difficulty)} mr-2">
                                                        ${map.difficulty || 'Không xác định'}
                                                    </span>
                                                    <i class="fas fa-clock mr-1"></i>
                                                    <span class="font-mono">${map.time || '--\'--\'--'}</span>
                                                </div>
                                            </div>
                                ...
                                ${racer.maps.length > 3 ? `
                                    <div class="text-center pt-2">
                                        <span class="text-xs text-slate-500">
                                            ...
                                            Và ${racer.maps.length - 3} bản đồ khác
                                        </span>
                                    </div>
                                ` : ''}
                            </div>
...
// Refresh top racers
window.refreshTopRacers = async () => {
    await loadTopRacers();
    showMessage("Đã làm mới danh sách tay đua!");
};
...
const renderNotifications = () => {
    ...
    if (filteredNotifications.length === 0) {
        notificationsList.innerHTML = `
            <div class="notification-empty">
                ...
                <h3 class="text-xl font-bold text-slate-400 mb-2">Không có thông báo</h3>
    ...
    filteredNotifications.forEach(notification => {
        ...
        notificationCard.className = `notification-card ${typeClass} ${unreadClass}`;
        ...
        notificationCard.innerHTML = `
                ...
                <div class="flex items-start flex-1">
                    ...
                    <div class="flex-1">
                        <div class="flex items-center gap-2 mb-2">
                            <h4 class="notification-title">${notification.title || 'Thông báo'}</h4>
                            ${notification.important ?
                '<span class="notification-badge important"><i class="fas fa-star mr-1"></i>QUAN TRỌNG</span>'
                : ''}
                            ${!notification.read ?
            ...
            <div class="notification-actions">
                ...
                ` : `
                    <span class="notification-meta-item text-green-400">
                        <i class="fas fa-check-circle"></i>
                        Đã đọc
                    </span>
                `}
                <button onclick="deleteNotification('${notification.id}', event)" 
                        class="notification-action-btn delete ml-auto">
                    <i class="fas fa-trash"></i>
                    Xóa
                </button>
            </div>
